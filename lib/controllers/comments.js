...
async function submit(request, response) {
  ...
  try {
    ...
    // Two auth paths: (1) comment_session cookie (IndieAuth visitor)
    //                  (2) Indiekit admin session (site owner)
    ...
    if (sessionId) {
      ...
      if (session) {
        commentUser = session.user;
      }
    }

    // Fall back to admin session (site owner commenting on own posts)
    if (!commentUser && request.session?.access_token) {
      const siteUrl = application.publication?.me || application.url;
      commentUser = {
        url: siteUrl,
        name: process.env.AUTHOR_NAME ||
          (siteUrl ? new URL(siteUrl).hostname : "Site Owner"),
 ...
 * Returns user info if authenticated, { user: null } otherwise
 */
...
async function isOwner(request, response) {
  ...
  try {
    ...
    const ownerName =
      ...
      (siteUrl ? new URL(siteUrl).hostname : "Site Owner");
    const ownerPhoto = process.env.AUTHOR_AVATAR || "";

    // Build syndication targets map keyed by service.name
    const syndicationTargets = {};
    if (publication?.syndicationTargets) {
      for (const target of publication.syndicationTargets) {
        const info = target.info;
        if (info?.uid && info?.service?.name) {
          syndicationTargets[info.service.name] = info.uid;
        }
      }
    }

    // Reply targets: maps detected platform to syndicator service.name.
    // Configurable via plugin options (replyTargets). Default maps each
    // platform to its matching syndicator name for external-account setups.
    const commentOptions = application.comments || {};
    const replyTargets = commentOptions.replyTargets || {
      mastodon: "Mastodon",
