...
export async function fetchProfile(url) {
  ...
  try {
    ...
    // Extract photo from h-card (src or value attr — hidden inputs are common)
    const photoMatch = html.match(
      /class="[^"]*u-photo[^"]*"[^>]*(?:src|value)="([^"]+)"/,
    );
    if (photoMatch) {
      profile.photo = photoMatch[1];
    } else {
      // Try img inside an h-card container
      const hcardImg = html.match(
        /class="[^"]*h-card[^"]*"[\s\S]*?<img[^>]+src="([^"]+)"/,
      );
      if (hcardImg) profile.photo = hcardImg[1];
    }
    ...
    if (!profile.photo) {
      ...
      if (ogImage) profile.photo = ogImage[1];
    }

    // Fallback: site icon (apple-touch-icon or favicon)
    if (!profile.photo) {
      ...
      if (touchIcon) {
        ...
        if (icon) profile.photo = icon[1];
      }
    }

    // Resolve relative photo URLs
    if (profile.photo && !profile.photo.startsWith("http")) {
      profile.photo = new URL(profile.photo, baseUrl).href;
    }

    // Fallback: name from meta author
    if (!profile.name) {
